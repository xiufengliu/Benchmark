...
public class Threelines implements Serializable {


    public static double[][] threel(List<Double> temperatures, List<Double> readings)  {
        try {
        TreeMap<Double, List<Double>> map = sort(temperatures, readings);
        int bin = 20;
        TreeMap<Double, Double> tempMap50 = binning(bin, 0.5f, map);
        double[] s = findSeparatorFirstRound(tempMap50);
        return conLines(tempMap50, s[0], s[1]);
        }catch (Exception e){
            return null;
        }
    }

    public static double[][] threelWritable(List<DoubleWritable> temperatures, List<DoubleWritable> readings)  {
        try {
            TreeMap<Double, List<Double>> map = sortWritable(temperatures, readings);
            int bin = 20;
            TreeMap<Double, Double> tempMap50 = binning(bin, 0.5f, map);
            double[] s = findSeparatorFirstRound(tempMap50);
            return conLines(tempMap50, s[0], s[1]);
    ...
    public static double[][] threel(String[] temperatures, String[] readings)  {
        TreeMap<Double, List<Double>> map = sort(temperatures, readings);
        int bin = 20;
        TreeMap<Double, Double> tempMap50 = binning(bin, 0.5f, map);
        double[] s = findSeparatorFirstRound(tempMap50);
    ...
    public static double[][] threel(TreeMap<Double, Double> tempMap50)  {
        double[] s = findSeparatorFirstRound(tempMap50);
        return conLines(tempMap50, s[0], s[1]);

    }

    public static TreeMap<Double, List<Double>> sort(double[] temperatures, double[] readings) {
        //build list for sorting
        TreeMap<Double, List<Double>> map = new TreeMap<Double, List<Double>>();
        for (int i = 0; i < temperatures.length; ++i) {
            double roundTemp = Math.round(temperatures[i]);
            if (!map.containsKey(roundTemp))
                map.put(roundTemp, new LinkedList<Double>());
            map.get(roundTemp).add(readings[i]);
        }
        return map;
    }

    public static TreeMap<Double, List<Double>> sortWritable( List<DoubleWritable> temperatures, List<DoubleWritable> readings) {
        //build list for sorting
        TreeMap<Double, List<Double>> map = new TreeMap<Double, List<Double>>();
        for (int i = 0; i < temperatures.size(); ++i) {
            double roundTemp = Math.round(temperatures.get(i).get());
            if (!map.containsKey(roundTemp))
                map.put(roundTemp, new LinkedList<Double>());
            map.get(roundTemp).add(readings.get(i).get());
        }
        return map;
    }

    public static TreeMap<Double, List<Double>> sort( List<Double> temperatures, List<Double> readings) {
        //build list for sorting
        TreeMap<Double, List<Double>> map = new TreeMap<Double, List<Double>>();
        for (int i = 0; i < temperatures.size(); ++i) {
            double roundTemp = Math.round(temperatures.get(i));
            if (!map.containsKey(roundTemp))
                map.put(roundTemp, new LinkedList<Double>());
            map.get(roundTemp).add(readings.get(i));
        }
        return map;
    }

    public static TreeMap<Double, List<Double>> sort( String[] temperatures, String[] readings) {
        //build list for sorting
        TreeMap<Double, List<Double>> map = new TreeMap<Double, List<Double>>();
        for (int i = 0; i < temperatures.length; ++i) {
            double roundTemp = Math.round(Double.parseDouble(temperatures[i]));
            if (!map.containsKey(roundTemp))
                map.put(roundTemp, new LinkedList<Double>());
            map.get(roundTemp).add(Double.valueOf(readings[i]));
        }
        return map;
    }
    ...
    public static TreeMap<Double, Double> binning(int i, double percentile, TreeMap<Double, List<Double>> map) {
        ...
        for (Entry<Double, List<Double>> entry : map.entrySet()) {
            ...
            if (readings.size() >= i) {
                double percent = percentile(percentile, readings);
                //double median= median(value);
                tempMap.put(temperature, percent);
            }
    ...
    public static double[][] conLines(TreeMap<Double, Double> data, double s1, double s2) {
        ...
        double min_RMSE = Double.MAX_VALUE;
