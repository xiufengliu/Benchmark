...
@Description(name = "par", value = "_FUNC_(meterid, reading) - emits () for the PAR programs")
public class UDTFPAR extends GenericUDTF {
    ...
    public StructObjectInspector initialize(ObjectInspector[] args) throws UDFArgumentException {
        ...
        if (args[0].getCategory() != ObjectInspector.Category.PRIMITIVE
            ...
            throw new UDFArgumentException("par() takes double as a parameter");
        }
    ...
    public void process(Object[] record) throws HiveException {
        ...
        if (preMeterID!=-1 && curMeterID.intValue()!=preMeterID){
            double[][] params = PAR.computeParameters3(readings, 3, 24);
            if (params!=null)
    ...
    public void close() throws HiveException {
        ...
        for (Map.Entry<Integer, double[][]> entry: resultMap.entrySet()){
            ...
            double[][] parameters = entry.getValue();
            buf.append("[");
            for (int i=0; i<parameters.length; ++i){
                double[] pars = parameters[i];
                buf.append("[");
